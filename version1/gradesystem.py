...
class Gradebook:
    """
    One Gradebook lives inside each section.
    Categories own assignment codes; together they replace the old sec_act set.
    """
    ...
    def add_assignment(self, code, category_id):
        """Attach an assignment code to a category."""
        if category_id not in self.categories:
            return False, "Category not found."
        # make sure code isn't already in another category
    ...
    def find_category_of(self, code):
        """Return the category_id that owns this assignment code, or None."""
    ...
    def all_assignments(self):
        """Return a set of all known assignment codes across all categories."""
        result = set()
    ...
    def calculate_final_grade(self, student_grades: dict):
        """
        student_grades: the student's grades dict {assignment_code: score}
        Returns (final_grade_float, breakdown_list)
        breakdown: list of (category_name, weight, avg_score, contribution)
        """
...
#  sections[sec]["gradebook"] is a Gradebook instance
#  sections[sec]["students"][sid]["grades"] = {assignment_code: score}
# ─────────────────────────────────────────────

def _make_section_gradebook():
    """Default gradebook for TN23/TN24 with assignments already mapped."""
    gb = Gradebook()
...
# Seed assignments into gradebooks so they match the grades already in the data
def _seed_assignments():
    for sec_data in sections.values():
        gb = sec_data["gradebook"]
        # SA1, SA2, SA3, SA4 → Seatwork
...
def add_section():
    sec = get_input("New section name: ")
    if sec in sections:
...
def add_assignment_to_section():
    """Add a new assignment code and assign it to a category in that section's gradebook."""
    section = select_section()
    if section not in sections:
        print("Invalid section!")
        return
...
def enter_assignment_scores(section):
    ...
    for sid in sections[section]["students"]:
        raw = get_input(f"  Score for {sid}: ")
        sections[section]["students"][sid]["grades"][a] = float(raw)
...
def edit_assignment_scores(section):
    ...
    for sid in sections[section]["students"]:
        ...
        if a in grades:
            grades[a] = float(get_input(f"  New score for {sid} (current {grades[a]}): "))
...
def edit_student_scores(section):
    ...
    for a in sorted(grades):
        grades[a] = float(get_input(f"  {a} (current {grades[a]}): "))
...
def calculate_and_display_final_grade(section, student_id):
    """Uses the section gradebook to compute the weighted final grade."""
    gb      = sections[section]["gradebook"]
...
def generate_report_card(name, scores):
    ...
    lines += [
        "+" + "-" * 38 + "+",
        f"| {'Average':<20} | {avg:>5.1f} | {simple_letter_grade(avg):<5} |",
        "+" + "=" * 38 + "+",
    ]
    return "\n".join(lines)
...
def admin_menu():
    while True:
        print_box([
            ...
            "5  Add Section",
            "6  Remove Section",
            "7  Add Assignment to Section",
            "8  Remove Assignment from Section",
            "9  View Full Table",
            "10 Gradebook (Categories & Assignments)",
            "11 Report Card Tools",
            "0  Exit",
        ])
        c = get_input("Choice: ")
        if c == "0":
            break
        {
            ...
            "5":  add_section,
            "6":  remove_section,
            "7":  add_assignment_to_section,
            "8":  remove_assignment_from_section,
            "9":  view_admin_table,
            "10": gradebook_menu,
            "11": report_card_menu,
        }.get(c, lambda: print("Invalid choice."))()
